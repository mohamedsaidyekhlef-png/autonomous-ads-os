...
export function AgentCommandCenter() {
  ...
  async function submitCommand() {
    ...
    try {
      ...
      const payload = contentType.includes("application/json")
        ? ((await response.json()) as Run & { detail?: string })
        : ({ detail: "The live AI endpoint returned an invalid response." } as Run & { detail?: string });
      if (!response.ok) throw new Error(typeof payload.detail === "string" ? payload.detail : "The campaign analysis could not be completed.");
      setRun(payload as Run);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "The campaign analysis could not be completed."); }
  }
  ...
  return <section className="overflow-hidden rounded-3xl border border-[#1468F3]/15 bg-white shadow-[0_18px_50px_rgba(20,104,243,0.08)]">
    <div className="border-b border-neutral-200 bg-gradient-to-r from-[#1468F3]/[0.07] to-[#FF6500]/[0.05] p-6">
      <div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#1468F3] text-white"><Bot className="h-6 w-6" /></span><div><div className="flex items-center gap-2"><h2 className="text-xl font-black text-neutral-950">Command your AI Ads Team</h2><Sparkles className="h-4 w-4 text-[#FF6500]" /></div><p className="mt-1 text-sm leading-6 text-neutral-600">Live Llama 3.3 70B campaign intelligence in protected Shadow Mode. No advertising action or spend is performed.</p></div></div>
      <div className="mt-5 rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm"><textarea value={command} onChange={(event) => setCommand(event.target.value)} placeholder="Describe your product, market, objective, budget, audience, landing page, and available metrics." rows={4} disabled={active} className="w-full resize-none border-0 bg-transparent px-2 py-2 text-base leading-7 text-neutral-950 outline-none placeholder:text-neutral-400" /><div className="flex flex-col gap-3 border-t border-neutral-100 pt-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-2 text-xs font-semibold"><span className="rounded-full bg-[#1468F3]/10 px-3 py-1.5 text-[#1468F3]">Shadow mode</span><span className="rounded-full bg-[#FF6500]/10 px-3 py-1.5 text-[#FF6500]">DRY_RUN mandatory</span></div><button type="button" onClick={submitCommand} disabled={active} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#FF6500] px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-60">{active ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}{active ? "Queued for specialists…" : "Analyze with Llama 3.3 70B"}</button></div></div>
    </div>
